...
def compute_directional_wrap_radius(
    ...
    # reject cross-section outliers per keypoint before theta binning
    keep = np.ones_like(rho, dtype=bool)

    for i in range(K):
        m = key_ids == i
        if np.sum(m) < max(10, min_count):
            continue

        r_med = np.median(rho[m])
        r_hi = np.quantile(rho[m], 0.90)

        # keep most local shell, reject neighboring/fold points
        keep[m] = rho[m] <= (r_hi * 1.10 + wrap_margin)
...
#    theta = theta[valid]
# reject points that are not close to the local cross-section
#    w_limit = float(wrap_w_factor) / max(K, 1)
#    valid = np.abs(w) <= w_limit
#
#    s = s[valid]
#    key_ids = key_ids[valid]
...
#
    ...
    support_count_s = np.zeros(K, dtype=np.int32)

    for i in range(K):
        mi = key_ids == i
        if not np.any(mi):
            continue
        support_count_s[i] = int(np.sum(mi))
        ...
        for j in range(n_theta_bins):
            ...
            wrap[i, j] = np.quantile(rho[m], quantile) * (1.0 + wrap_relative_margin) + wrap_margin
    ...
    # Do not let unsupported tail sections grow huge after surface points end.
    min_support_s = max(min_count * 2, 20)
    valid_s = support_count_s >= min_support_s

    if np.any(valid_s):
        first_valid = int(np.where(valid_s)[0][0])
        last_valid = int(np.where(valid_s)[0][-1])

        # before first support: hold first valid wrap
        wrap[:first_valid] = wrap[first_valid]

        # after last support: hold last valid wrap
        wrap[last_valid + 1:] = wrap[last_valid]
        # apply raw safety only inside supported interval
        wrap[first_valid:last_valid + 1] = np.maximum(
            wrap[first_valid:last_valid + 1],
            0.85 * wrap_raw_interp[first_valid:last_valid + 1],
        )

    # remove one-section rogue explosions
#    row_med = np.median(wrap, axis=1)
#
#    for i in range(1, K - 1):
#        neigh = 0.5 * (row_med[i - 1] + row_med[i + 1])
#        if row_med[i] > 1.35 * neigh:
#            wrap[i] = 0.5 * (wrap[i - 1] + wrap[i + 1])
#    wrap = np.maximum(wrap, 0.98 * wrap_raw_interp)
...
def main():
    ...
    p.add_argument("--smooth_keypoints_sigma", type=float, default=0.0)
    p.add_argument("--smooth_keypoints_preserve_endpoints", action="store_true")
