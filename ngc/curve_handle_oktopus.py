...
class CurveHandle():
    ...
    def update(self):
        # Set relative coordinate frame and roation slerp between the keypoints
        self.core.update()
        #n_sample_curve = 20
        n_sample_curve = self.core.key_points.shape[0]
        n_sample_circle = 12
    ...
    def gen_cylinder_mesh(self, n_sample_curve, n_sample_circle):
        ...
        # Better not to have the sample curve but take all the points in the key_point detector as such
        # reduces the amoutn of time to do this.
        ...
        intpl = {}
        intpl['points'] = self.core.key_points
        intpl['radius'] = self.core.key_radius
        intpl['frame'] = self.core.rot_slerp(self.core.key_ts).as_matrix()

        #intpl = self.core.interpolate(self.key_ts)
        intpl['thetas'] = thetas
    ...
    def __gen_cyl_mesh(self, intpl):
        ...
        #print("scales = ", scales)
        cyl_pts *= scales[..., None]
        #print("cyl pts = ", cyl_pts)

        # Obtaining vertex positions on the circle as a projection of the 
        # key point to the circle (keyframe circle)
        #cyl_pts = np.array(cyl_pts).astype(float)
        cyl_pts += np.array(verts[:, None, :], dtype=np.float64)#.astype(float)
        #print("cyl pts +verts= ", cyl_pts)
...
#            v1 += disp
        ...
        # the vertex indices of cylinder for the curve
    ...
    def calc_cylinder_SDF(self, mc_grid):
        # For each cylidrical part of a segment, create a mc grid
    ...
    def find_inside(self, points):
        ...
        ## The samples that are inside the local coord systems
...
#Piece-wise linear approximation curve
class PWLACurve():
    ...
    def update(self):
        if self.flag_points:
            # Update the absolute coordinates to relative coordinate frame from [0-1]
            self.update_coords()
            # Set the rotation interpolation of the frame of the curve
            self.update_frame()
    ...
    def set_curve(self, arg):
        ...
        self.key_radius = arg['key_radius']+0.01
...
#                self.start_ball_x = None
        ...
        x_axis = arg['x_axis'] #self.estimate_tangent(self.key_points)
        self.x_axis = x_axis
        #x_axis = self.estimate_tangent(self.key_points)
        #x_axis = self.estimate_tangent(self.key_points)
        self.y_axis = arg['y_axis']
        z_axis = arg['z_axis']
    ...
    def calc_curve_length(self, points):
        pts = points # self.key_points
    ...
    def set_resamples(self, points, z_axis0):
        ...
        #edge_lengths = np.linalg.norm(edge_vec, axis=1)
        curve_length = self.calc_curve_length(points) #np.sum(edge_lengths)
    ...
    def project_z_axis(self, x_axis, z_axis):
        ...
        print("pre projection of z-axis")
        print(dots)
        if not np.allclose(dots, 0):
            # project z_axis to x_axis
            print("not all is close to 0")
    ...
    def update_frame(self):
        x_axis = self.x_axis #self.estimate_tangent(self.key_points)
        #x_axis = self.estimate_tangent(self.key_points)
        #self.z_axis = self.project_z_axis(x_axis, self.z_axis)
        y_axis = self.y_axis #np.cross(self.z_axis, x_axis)
        #y_axis = np.cross(self.z_axis, x_axis)
        #y_axis = self.y_axis
        self.key_frame = np.concatenate([
            x_axis.reshape(-1,1,3),
    ...
    # this function makes uniform keypoint location on linear scale [0,1] all along the curve
    # this might not be necessary

    def update_coords(self):
        #edge_vec = np.array(self.key_points[1:] - self.key_points[:-1])#.astype(float)
        edge_vec = np.array(self.key_points[1:] - self.key_points[:-1], dtype=np.float64)#.astype(float)
        #print("edge vec = ", len(edge_vec))
        #edge_lengths = []
        #for i,ev in enumerate(edge_vec):
        #    print(i)
        #    print(len(ev))
        #    print(ev)
            #print(i)
            #print(np.linalg.norm(ev))
            #edge_lengths.append(np.linalg.norm(ev))
        edge_lengths = np.linalg.norm(edge_vec, axis=1)
        self.curve_length = np.sum(edge_lengths)
        self.key_ts = np.cumsum(np.r_[0., edge_lengths]) / self.curve_length
        #print(self.key_points)
        print("key ts shape = ", self.key_ts.shape)
        print(self.key_points)
        #exit()
    ...
    def curve_projection(self, samples, N_discrete=20, outside=False):
        ...
        for vid in range(num_vert):
            ### Get all the samples that correspond to key point index vid
            ...
            if 0 < vid < num_vert - 1:
                # middle part
                ### Mark all the samples with the corresponding keypoint spacing that falls within linspace(0,1)
                ts[sidx] = td[vid]
                ...
                #print("in1 =", in1)
                #print("verts[vid] =", verts[vid])
                #print("verts[vid+1] =", verts[vid+1])
                #print("px1 = ", px1)
                #exit()
                ...
                # In either the section of [vid - vid+1] or [vid-1 - vid]
                in_p = np.logical_xor(in1, in2)
                # Get the exact location
                px = (in1*px1 + in2*px2)[in_p]
                # Update the keypoint location with the exact location
                ts[sidx[in_p]] = px
    ...
    def calc_cylinder_SDF(self, vs):
        # Project all the sample points on to the curve
        ts = self.curve_projection(vs, outside=True)

        # With the newly projected points, get all the key points of the curve
        # i.e, it stared with random keypoints on a curve
        # then added linspace of 20 equally spaced keypoints
        # then projected sampled on to the curve
        # finally interpolate for all the keypoint values, radius and frames

        intpl = self.interpolate(ts)
        # projected samples interpolation
        proj_vs = intpl['points']
        yz_rs = intpl['radius']
        # the low twist frame co-ord for each key point
        frame_mat = intpl['frame']

        x_rs = self.calc_x_radius(ts)

        ## key frame radius (x, y, z) for each key points
        ...
        #print("proj vs = ", proj_vs.shape)
        #exit()


        # convert samples to local frame ?? 
        samples_local = np.einsum('nij,nj->ni', frame_mat, (vs - proj_vs))
        #print(samples_local.shape)
        #pcls = []
        #pcls.append({'type': 'points', 'vertices':vs})
        #pcls.append({'type': 'points', 'vertices':proj_vs})
        #pcls.append({'type': 'points', 'vertices':samples_local})
        #visualize(pcls)
        #exit()
        ...
        # move points between -1 and 1
        vx = 2*ts - 1
        # move x between -1 and 1
        samples_local[:, 0] += vx
        ...
        # convert it back to range 0 to 1
        return norms_cyl - 1.
    ...
    def localize_samples(self, vs, return_sdf=False):
        ...
        # ts_range is boolean
        ts = ts[ts_range]
        # ts the projection points of the sample on to the curve
        #print("ts = ", ts)
        #exit()
        vs = vs[ts_range]
        sidx = sidx[ts_range]
    ...
    def interpolate(self, ts, points=True, radius=True, frame=True):
        ...
        #print("ts = ", ts)
        #print("self key points ts = ", self.key_ts)
        #print("self key points = ", self.key_points[:,0])
        #print(np.interp(ts, self.key_ts, self.key_points[:, 0]))
        #exit()
        #ts = np.array(ts).astype(float)
        #self.key_ts = np.array(self.key_ts).astype(float)
        #self.key_points = np.array(self.key_points).astype(float)
        ...
        if points:
            ...
            #print(self.key_ts)
            #print("***")
            #print(self.key_points[:,0])
            print('x dtype:', np.array(ts).dtype)
            print('xp dtype:', np.array(self.key_ts).dtype)
            #if isinstance(np.array(self.key_points[:,0]).dtype, object):
            #    print(self.key_points[:,0].dtype)
            #    print(self.key_points[:,0])
            self.key_points = np.array(self.key_points, dtype=np.float64)
            #self.key_points[:,1] = np.array(self.key_points[:,1], dtype=np.float64)
            #self.key_points[:,2] = np.array(self.key_points[:,2], dtype=np.float64)
            print('fp0 dtype:', np.array(self.key_points[:,0]).dtype)
            print('fp1 dtype:', np.array(self.key_points[:,1]).dtype)
            print('fp2 dtype:', np.array(self.key_points[:,2]).dtype)
            print("END")
            pts_ts = np.stack([
