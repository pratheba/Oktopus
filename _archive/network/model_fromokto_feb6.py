...
class NGCNet(nn.Module):
    ...
    def forwardsimple(self, model_input, curr_epoch=0, istrain=True):
        ...
        if istrain:
            ...
            if 'coords_detail' in mi:
                curve_coords_detail_posenc = self.pos_enc_curve(mi['coords_detail'].unsqueeze(-1))
        ...
        curve_code_coords_detail = torch.cat([curve_code, curve_coords_detail_posenc, radii_y_posenc, radii_z_posenc], dim=-1)
        curve_feats = self.curveencoder(curve_code_coords) 
        curve_feats_detail = self.curveencoder(curve_code_coords_detail) 
        ...
        if not self.issplit:
            ...
            x = self.filmenc_detail1(y, curve_feats_detail) + type_sample
            res = x
